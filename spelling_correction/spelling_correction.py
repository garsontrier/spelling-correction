...
def get_corpus(corpus_path='../'):  # Tokenizes corpus and returns it
...
def get_spelling_errors(spell_path='../'):
...
if __name__ == '__main__':
    ...
    for k in smoothing:
        ...
        if save_imgs:
            df = pd.DataFrame(data=c_deletion, columns=get_alphabet(),
                              index=get_alphabet())
            plt.figure(figsize=(15, 15))
            sn.heatmap(df, annot=True)
            plt.show()
            plt.savefig('deletion_' + txt + '.png')

            df = pd.DataFrame(data=c_insertion, columns=get_alphabet(),
                              index=get_alphabet())
            plt.figure(figsize=(15, 15))
            sn.heatmap(df, annot=True)
            plt.show()
            plt.savefig('insertion_' + txt + '.png')

            df = pd.DataFrame(data=c_subs, columns=get_alphabet(),
                              index=get_alphabet())
            plt.figure(figsize=(15, 15))
            sn.heatmap(df, annot=True)
            plt.show()
            plt.savefig('subs_' + txt + '.png')


            df = pd.DataFrame(data=c_trans, columns=get_alphabet(),
                              index=get_alphabet())
            plt.figure(figsize=(15, 15))
            sn.heatmap(df, annot=True)
            plt.show()
            plt.savefig('trans_' + txt + '.png')
        common = 0
